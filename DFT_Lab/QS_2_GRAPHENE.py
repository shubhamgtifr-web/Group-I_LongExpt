...
Original file is located at
    https://colab.research.google.com/drive/14Iqzfa8et4aqUdOm-IL5gXvCa9Ag7hyr
"""
...
path_points = [GAMMA, M_pt, K_pt, GAMMA]
labels_seq  = [r'$\Gamma$', 'M', 'K', r'$\Gamma$']

HR_FILE = Path("graphene_hr.dat")
POSCAR = Path("POSCAR_graphene.txt")
...
def main():
    ...
    klist_frac = []
    label_positions = []  # indices of HS points

    for i, (start, end) in enumerate(zip(path_points[:-1], path_points[1:])):
        endpoint = (i == len(path_points) - 2)
        seg = [
            start * (1 - t) + end * t
            for t in np.linspace(0, 1, points_per_segment, endpoint=endpoint)
        ]
    ...
    plt.xticks(tick_pos, labels_seq)
    for tp in tick_pos:
        plt.axvline(tp, linestyle=':', linewidth=0.6)

    plt.ylabel("Energy (eV)")
    plt.title(f"Band structure of Graphene")
    plt.tight_layout()
    plt.show()
